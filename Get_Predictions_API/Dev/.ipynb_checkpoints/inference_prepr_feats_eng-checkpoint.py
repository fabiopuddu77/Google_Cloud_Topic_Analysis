...
class GCS_preprocessing:
    ...
    def preprocess_articles(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df[self.text_column] = df['body'].apply(self.preprocess_text)
        return df
    ...
    def process(self):
        if not self.overwrite and self.file_exists_in_gcs():
            ...
            self.logger.info("Dataframe final saved to GCS")
            
            self.upload_dataframe_to_bigquery(df_entity)
            self.logger.info(" Dataframe finale saved to Big Query")
            return self.pred()
